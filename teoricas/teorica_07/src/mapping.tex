...
\begin{frame}
    ...
    \note{Content from https://youtu.be/v-Rm9TUG9LA}
    ...
    \note{A \emph{forward model} predicts sensor readings based on known system parameters, while an \emph{inverse model} estimates system parameters from observed sensor readings.}

    \note{A forward sensor model, given a hypothetical occupancy grid map, would predict the sonar readings the robot should observe. It would consider all occupied cells within the sonar cone. For example, it might calculate the probability of detecting the closest obstacle, the second closest (if the first isn't detected), or even a random reading due to sensor failure or specular reflection.}

    \note{More info: forward measurement model vs inverse measurement model https://www.cs.cmu.edu/~thrun/papers/thrun.occ-journal.pdf} 

    \note{More info: http://robots.stanford.edu/papers/thrun.iros01-occmap.pdf}
\end{frame}
...
\begin{frame}
    ...
    \begin{center}
        \movie[autostart,loop,poster]{\includegraphics[width=0.8\columnwidth]{./images/occupancy_grid_map_video.jpg}}{./videos/occupancy_grid_map.mp4}
    \end{center}
